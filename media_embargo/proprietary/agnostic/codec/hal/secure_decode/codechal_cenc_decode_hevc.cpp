...
*/
//!
//! \file      codechal_cenc_decode_hevc.cpp 
...
CodechalCencDecodeHevc::CodechalCencDecodeHevc():CodechalCencDecode()
{

}

CodechalCencDecodeHevc::~CodechalCencDecodeHevc()
{
...
MOS_STATUS CodechalCencDecodeHevc::Initialize(
    ...
    CodechalSetting             *settings)
{
    ...
    CENC_DECODE_CHK_STATUS(m_hcpInterface->GetHcpPrimitiveCommandSize(
...
MOS_STATUS CodechalCencDecodeHevc::AllocateDecodeResource(
    uint8_t bufIdx)
{
    CENC_DECODE_FUNCTION_ENTER;
...
MOS_STATUS CodechalCencDecodeHevc::InitializeAllocation()
{
    MOS_STATUS eStatus = MOS_STATUS_SUCCESS;
...
MOS_STATUS CodechalCencDecodeHevc::InitializeSharedBuf(
...
MOS_STATUS CodechalCencDecodeHevc::UpdateSecureOptMode(
...
MOS_STATUS CodechalCencDecodeHevc::SetDecodeParams(
...
void CodechalCencDecodeHevc::ScalingListSpsPps(
...
void CodechalCencDecodeHevc::ScalingListDefault(
    PCODECHAL_HEVC_IQ_MATRIX_PARAMS  hevcQmParams)
{
    uint8_t   idx;
    uint8_t*  hevcDstMatrix;
...
void CodechalCencDecodeHevc::ScalingListFlat(
    PCODECHAL_HEVC_IQ_MATRIX_PARAMS  hevcQmParams)
{
    uint8_t i;
...
MOS_STATUS CodechalCencDecodeHevc::PackProfileTier(
...
MOS_STATUS CodechalCencDecodeHevc::PackPTL(
...
MOS_STATUS CodechalCencDecodeHevc::PackScalingMatrix(
    uint8_t sizeNum, 
    uint8_t matrixSize,
...
MOS_STATUS CodechalCencDecodeHevc::PackScalingList(
...
MOS_STATUS CodechalCencDecodeHevc::PackShortTermRefPicSet(
...
MOS_STATUS CodechalCencDecodeHevc::PackHrdParameters(
...
MOS_STATUS CodechalCencDecodeHevc::PackVUI(
...
MOS_STATUS CodechalCencDecodeHevc::EncodeSPS(
...
MOS_STATUS CodechalCencDecodeHevc::EncodePPS(
    const PCODECHAL_DECRYPT_STATUS_REPORT_HEVC statusDriver, 
    uint32_t bufSize, 
    uint8_t* &outBitStreamBuf, 
    uint32_t &writtenBytes)
...
MOS_STATUS CodechalCencDecodeHevc::EncodeSeiBufferingPeriod(
    const PCODECHAL_DECRYPT_STATUS_REPORT_HEVC statusDriver, 
...
MOS_STATUS CodechalCencDecodeHevc::EncodeSeiPicTiming(
    const PCODECHAL_DECRYPT_STATUS_REPORT_HEVC statusDriver, 
...
MOS_STATUS CodechalCencDecodeHevc::EncodeSeiWriteMessage(
...
MOS_STATUS CodechalCencDecodeHevc::EncodeSEI(
    const PCODECHAL_DECRYPT_STATUS_REPORT_HEVC statusDriver, 
    uint32_t bufSize, 
    uint8_t* &outBitStreamBuf, 
    uint32_t &writtenBytes)
