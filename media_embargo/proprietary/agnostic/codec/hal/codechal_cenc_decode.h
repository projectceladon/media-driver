...
public:
    ...
    //!
    CodechalCencDecode(const CodechalCencDecode&) = delete;

    //!
    //! \brief    Copy assignment operator
    //!
    CodechalCencDecode& operator=(const CodechalCencDecode&) = delete;

    //!
    //! \brief  Destructor
    //!
    virtual ~CodechalCencDecode();
    ...
    //!         MOS_STATUS_SUCCESS if success, else fail reason
    //!
    virtual MOS_STATUS GetStatusReport(void *status, uint16_t numStatus, HUC_STATUS *hucStatus = nullptr);
    ...
    //!         MOS_STATUS_SUCCESS if success, else fail reason
    //!
    virtual MOS_STATUS UpdateDataBuffer(void* buffer, uint32_t size);
