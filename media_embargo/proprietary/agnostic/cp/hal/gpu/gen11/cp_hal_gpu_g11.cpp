...
Platform specific - should support - Gen 11
...
#define PAVP_GEN11_DECODE_SESSION_IN_PLAY_MASK       0xFFFF
#define PAVP_GEN11_TRANSCODE_SESSION_IN_PLAY_MASK    0xFF0000
#define PAVP_GEN11_WIDI_SESSION_IN_PLAY_MASK         0x1000
...
CPavpGpuHal11::CPavpGpuHal11(PLATFORM& sPlatform, MEDIA_WA_TABLE& sWaTable, HRESULT& hr, CPavpOsServices& OsServices)
...
HRESULT CPavpGpuHal11::BuildKeyExchange(
...
int32_t CPavpGpuHal11::BuildMultiCryptoCopy(
...
HRESULT CPavpGpuHal11::BuildCryptoCopy(
    MOS_COMMAND_BUFFER                      *pCmdBuf,
    const PAVP_GPU_HAL_CRYPTO_COPY_PARAMS&  sBuildCryptoCopyParams)
{
    ...
    {
        hr = BuildCryptoCopy(pCmdBuf, sBuildCryptoCopyParams, true);
    }
...
bool CPavpGpuHal11::CheckAudioCryptoCopyCounterRollover(
    ...
    uint32_t                              &remainingSize)
{
    ...
    if (bSplitCryptoCopy)
    {
        ...
        for (size_t i = 0; i < overflowBits.size(); i++)
        {
            if (overflowBits[i])
            {
                counterAfterRollover |= (1ULL << (i + PAVP_GEN11_FIRST_RESERVED_COUNTER_BIT));
            }
        }
    ...
    const BOOL bWaitForCompletion)
{
    ...
    do
    {
        ...
        if (CryptoCopyAddrParams.size() != CryptoCopyParams.size())
        {
            ...
            hr = E_UNEXPECTED;
            MT_ERR3(MT_CP_CRYPT_COPY_PARAM, MT_CP_CRYPT_COPY_ADDR_CMD, static_cast<int64_t>(CryptoCopyAddrParams.size()), MT_CP_CRYPT_COPY_CMD,
                                                                             static_cast<int64_t>(CryptoCopyParams.size()), MT_ERROR_CODE, hr);
            break;
        }
...
HRESULT CPavpGpuHal11::SetGpuCryptoKeyExParams(PMHW_PAVP_ENCRYPTION_PARAMS pMhwPavpEncParams)
{
    HRESULT hr = S_OK;
    MOS_STATUS  eStatus = MOS_STATUS_UNKNOWN;
    ...
    do {
        if (pMhwPavpEncParams == NULL)
        {
            GPU_HAL_ASSERTMESSAGE("NULL ptr!");
            hr = E_INVALIDARG;
            MT_ERR2(MT_ERR_NULL_CHECK, MT_CODE_LINE, __LINE__, MT_ERROR_CODE, hr);
            break;
        }
...
HRESULT CPavpGpuHal11::BuildQueryStatus(
    PMOS_COMMAND_BUFFER                      pCmdBuf,
...
HRESULT CPavpGpuHal11::SetKeyRefreshType()
{
    ...
    GPU_HAL_CHK_STATUS_WITH_HR(m_pOsServices->ReadMMIORegister(PAVP_REGISTER_KCR_APP_CHAR, ValueRead, CPavpGpuHal::ConvertRegType2RegOffset(PAVP_REGISTER_KCR_APP_CHAR)));
    
    GPU_HAL_NORMALMESSAGE("PAVP_REGISTER_KCR_APP_CHAR = 0x%x",ValueRead);

    if (ValueRead & PAVP_GEN11_KCR_APP_CHAR_KEY_REFRESH_MODE_BIT_MASK)
    {
...
BOOL CPavpGpuHal11::CheckCryptoCopyCounterRollover(
    uint32_t DataLength,
    uint32_t CounterLow,
    ...
    std::array<bool, 4>& OverflowBits)
{
    ...
    for (int i = PAVP_GEN11_FIRST_RESERVED_COUNTER_BIT; i <= PAVP_GEN11_LAST_RESERVED_COUNTER_BIT; i++)
    {
        WillRollover |= (InitialCounterBits[i] == 0 && FinalCounterBits[i] == 1);
        uint32_t index = i - PAVP_GEN11_FIRST_RESERVED_COUNTER_BIT;
    ...
    const BOOL OverflowInBit63 = (InitialCounterBits[PAVP_GEN11_LAST_RESERVED_COUNTER_BIT] == 1 &&
        FinalCounterBits[PAVP_GEN11_LAST_RESERVED_COUNTER_BIT] == 0);
    WillRollover |= OverflowInBit63;
    if (OverflowInBit63 == TRUE)
    {
        OverflowBits[PAVP_GEN11_LAST_RESERVED_COUNTER_BIT - PAVP_GEN11_FIRST_RESERVED_COUNTER_BIT] = true;
    }
