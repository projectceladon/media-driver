...
CPavpGpuHal12::CPavpGpuHal12(PLATFORM& sPlatform, MEDIA_WA_TABLE& sWaTable, HRESULT& hr, CPavpOsServices& OsServices)
...
HRESULT CPavpGpuHal12::BuildKeyExchange(
...
int32_t CPavpGpuHal12::BuildMultiCryptoCopy(
...
bool CPavpGpuHal12::CheckAudioCryptoCopyCounterRollover(
    ...
    uint32_t                              &remainingSize)
 {
     ...
     if (bSplitCryptoCopy)
     {
         ...
         for (size_t i = 0; i < overflowBits.size(); i++)
         {
             if (overflowBits[i])
             {
                 counterAfterRollover |= (1ULL << (i + PAVP_GEN12_FIRST_RESERVED_COUNTER_BIT));
             }
         }
 ...
 HRESULT CPavpGpuHal12::BuildCryptoCopy(
    MOS_COMMAND_BUFFER                      *pCmdBuf,
...
HRESULT CPavpGpuHal12::SetGpuCryptoKeyExParams(PMHW_PAVP_ENCRYPTION_PARAMS pMhwPavpEncParams)
{
    HRESULT hr = S_OK;
    MOS_STATUS  eStatus = MOS_STATUS_UNKNOWN;
...
HRESULT CPavpGpuHal12::BuildQueryStatus(
    PMOS_COMMAND_BUFFER                      pCmdBuf,
...
HRESULT CPavpGpuHal12::SetKeyRefreshType()
{
...
BOOL CPavpGpuHal12::CheckCryptoCopyCounterRollover(
    uint32_t DataLength,
    uint32_t CounterLow,
    ...
    std::array<bool, 4>& OverflowBits)
{
    ...
    for (int i = PAVP_GEN12_FIRST_RESERVED_COUNTER_BIT; i <= PAVP_GEN12_LAST_RESERVED_COUNTER_BIT; i++)
    {
        WillRollover |= (InitialCounterBits[i] == 0 && FinalCounterBits[i] == 1);
        uint32_t index = i - PAVP_GEN12_FIRST_RESERVED_COUNTER_BIT;
    ...
    const BOOL OverflowInBit63 = (InitialCounterBits[PAVP_GEN12_LAST_RESERVED_COUNTER_BIT] == 1 &&
        FinalCounterBits[PAVP_GEN12_LAST_RESERVED_COUNTER_BIT] == 0);
    WillRollover |= OverflowInBit63;
    if (OverflowInBit63 == TRUE)
    {
        OverflowBits[PAVP_GEN12_LAST_RESERVED_COUNTER_BIT - PAVP_GEN12_FIRST_RESERVED_COUNTER_BIT] = true;
    }
