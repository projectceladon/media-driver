...
*/
//!
//! \file      codechal_cenc_decode_avc_next.cpp
...
CodechalCencDecodeAvcNext::CodechalCencDecodeAvcNext() : CodechalCencDecodeNext()
{

}

CodechalCencDecodeAvcNext::~CodechalCencDecodeAvcNext()
{
...
MOS_STATUS CodechalCencDecodeAvcNext::Initialize(
    ...
    CodechalSetting             *settings)
{
    ...
    CENC_DECODE_CHK_STATUS(m_hwInterface->GetMfxPrimitiveCommandsDataSize(
        settings->mode,
...
MOS_STATUS CodechalCencDecodeAvcNext::AllocateDecodeResource(
    uint8_t bufIdx)
{
    CENC_DECODE_FUNCTION_ENTER;
...
MOS_STATUS CodechalCencDecodeAvcNext::SetDecodeParams(
...
int32_t CodechalCencDecodeAvcNext::GetMaxFSFromLevelIdc(
    int32_t     levelIdc)
{
    int32_t     frameSize = 0;
...
uint32_t CodechalCencDecodeAvcNext::GetMaxSupportedSlices(
    uint16_t    mbHeight)
{
...
uint32_t CodechalCencDecodeAvcNext::GetMaxBitstream(
    uint16_t mbWidth,
    uint16_t mbHeight)
{
...
void CodechalCencDecodeAvcNext::ScalingList(
...
void CodechalCencDecodeAvcNext::ScalingListFallbackA(
...
void CodechalCencDecodeAvcNext::ScalingListFallbackB(
    PCODEC_AVC_IQ_MATRIX_PARAMS     avcQmParams,
    uint8_t                         *inputScalingList,
    uint8_t                         qmIdx)
{
...
uint32_t CodechalCencDecodeAvcNext::VerifySliceHeaderSetParams(
...
MOS_STATUS CodechalCencDecodeAvcNext::InitializeSharedBuf(
...
MOS_STATUS CodechalCencDecodeAvcNext::UpdateSecureOptMode(
...
MOS_STATUS CodechalCencDecodeAvcNext::InitializeAllocation()
{
    MOS_STATUS  eStatus = MOS_STATUS_SUCCESS;
...
bool CodechalCencDecodeAvcNext::IsFRextProfile(uint8_t profile)
{
...
MOS_STATUS CodechalCencDecodeAvcNext::PackScalingList(
    uint8_t  *scalingList,
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSPS(
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodePPS(
    const PCODECHAL_DECRYPT_STATUS_REPORT_AVC statusDriver, 
    uint32_t                                  bufSize,
    uint8_t                                   *&outBitStreamBuf,
    uint32_t                                  &writtenBytes)
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSeiBufferingPeriod(
    const PCODECHAL_DECRYPT_STATUS_REPORT_AVC statusDriver, 
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSeiPicTiming(
    const PCODECHAL_DECRYPT_STATUS_REPORT_AVC statusDriver, 
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSeiRecoveryPoint(
    const PCODECHAL_DECRYPT_STATUS_REPORT_AVC statusDriver, 
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSeiWriteMessage(
...
MOS_STATUS CodechalCencDecodeAvcNext::EncodeSEI(
    const PCODECHAL_DECRYPT_STATUS_REPORT_AVC statusDriver, 
    uint32_t                                  bufSize,
    uint8_t                                   *&outBitStreamBuf,
    uint32_t                                  &writtenBytes)
