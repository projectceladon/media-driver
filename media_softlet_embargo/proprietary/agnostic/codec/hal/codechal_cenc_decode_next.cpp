...
*/
//!
//! \file      codechal_cenc_decode_next.cpp
...
CodechalCencDecodeNext::CodechalCencDecodeNext()
{
...
CodechalCencDecodeNext::~CodechalCencDecodeNext()
{
    ...
    for (int i = 0; i < CODECHAL_NUM_DECRYPT_BUFFERS; i++)
    {
        ...
        {
            // for not M12+, free 2nd level batch buffer
    ...
    if (m_mhwInterfaces)
    {
        // Already deleted in m_hwInterface
...
MOS_STATUS CodechalCencDecodeNext::Create(
    ...
    CodechalCencDecodeNext **cencDecoder)
{
    ...
    {
    case CODECHAL_AVC:
        *cencDecoder = MOS_New(CodechalCencDecodeAvcNext);
        break;
    case CODECHAL_HEVC:
        *cencDecoder = MOS_New(CodechalCencDecodeHevcNext);
        break;
    default:
...
MOS_STATUS CodechalCencDecodeNext::VerifySpaceAvailable()
{
    MOS_STATUS eStatus = MOS_STATUS_SUCCESS;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        else
        {
            if (m_osInterface->bUsesCmdBufHeaderInResize)
            {
                ...
                auto &miNoopParams = m_miItf->MHW_GETPAR_F(MI_NOOP)();
                miNoopParams       = {};
                CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MI_NOOP)(&cmdBuffer));
...
MOS_STATUS CodechalCencDecodeNext::SendPrologWithFrameTracking(
    PMOS_COMMAND_BUFFER cmdBuffer)
{
    ...
    CENC_DECODE_CHK_STATUS(Mhw_SendGenericPrologCmdNext(cmdBuffer, &genericPrologParams, m_miItf));

    return eStatus;
}

MOS_STATUS CodechalCencDecodeNext::AllocateResources()
{
    MOS_STATUS                   eStatus = MOS_STATUS_SUCCESS;
    ...
    *lockedTracker  = MemoryBlock::m_invalidTrackerId;
    ...
    if (MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
    {
        // for M12+ set initial size to 2048K=32*64K=512*CODECHAL_PAGE_SIZE to store at least 32 frame 64K batch buffer in typical netflix avc usage to avoid heap resize
    ...
    if (!MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
    {
        // for not M12+, need clean m_resSlcLevelBatchBuf before use
        MOS_ZeroMemory(
    ...
    for (i = 0; i < CODECHAL_NUM_DECRYPT_BUFFERS; i++)
    {
        if (!MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
        {
            // for not M12+, allocate 2nd level batch buffer, assuming 4k resolution
        ...
        allocParamsForBufferLinear.ResUsageType = MOS_HW_RESOURCE_USAGE_CP_EXTERNAL_READ;
        CENC_DECODE_CHK_STATUS(m_osInterface->pfnAllocateResource(
            m_osInterface,
            &allocParamsForBufferLinear,
...
MOS_STATUS CodechalCencDecodeNext::SetHucDmemParams()
{
    MOS_LOCK_PARAMS     lockFlags;
    ...
    ReadUserSettingForDebug(m_userSettingPtr,
        outValue,
        "Fulsim In Use",
        MediaUserSetting::Group::Frame);
    decryptDmem->uiFulsimInUse = outValue.Get<int32_t>();
#endif
...
MOS_STATUS CodechalCencDecodeNext::SetHucDashAesSegmentInfo(
    ...
    PMOS_COMMAND_BUFFER      cmdBuffer)
{
    ...
    // MHW_ADDCMD_F will override protectionSetting bit based on cp status
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MFX_WAIT)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_PIPE_MODE_SELECT)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MFX_WAIT)(cmdBuffer));

    auto &indObjParams                     = m_hucItf->MHW_GETPAR_F(HUC_IND_OBJ_BASE_ADDR_STATE)();
    indObjParams                           = {};
    indObjParams.DataBuffer                = resSegmentInfo;
    indObjParams.DataSize                  = sizeof(CODECHAL_HUC_SEGMENT_INFO) * params->dwNumSegments;
    indObjParams.StreamOutObjectBuffer     = params->presCryptoBuffer;
    indObjParams.StreamOutObjectSize       = sizeof(CODECHAL_HUC_SEGMENT_INFO) * params->dwNumSegments;

    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_IND_OBJ_BASE_ADDR_STATE)(cmdBuffer));

    auto &streamObjParams                         = m_hucItf->MHW_GETPAR_F(HUC_STREAM_OBJECT)();
    streamObjParams                               = {};
    streamObjParams.IndirectStreamInDataLength    = sizeof(CODECHAL_HUC_SEGMENT_INFO) * params->dwNumSegments;
    streamObjParams.IndirectStreamInStartAddress  = 0;
    streamObjParams.IndirectStreamOutStartAddress = 0;
    streamObjParams.StreamOut                     = 1;
     
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_STREAM_OBJECT)(cmdBuffer));
...
//!
MOS_STATUS CodechalCencDecodeNext::UpdateGlobalCmdBufId(
    ...
    CODECHAL_CENC_HEAP_INFO       *heapInfo)
{
    ...
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MI_STORE_DATA_IMM)(cmdBuffer));
...
//!           MOS_STATUS_SUCCESS if success, else fail reason
//!
MOS_STATUS CodechalCencDecodeNext::UpdateCurrCmdBufId()
{
    MOS_STATUS eStatus = MOS_STATUS_SUCCESS;
...
//!           MOS_STATUS_SUCCESS if success, else fail reason
//!
MOS_STATUS CodechalCencDecodeNext::AssignSyncTag()
{
    MOS_STATUS eStatus = MOS_STATUS_SUCCESS;
...
//!           MOS_STATUS_SUCCESS if success, else fail reason
//!
MOS_STATUS CodechalCencDecodeNext::UpdateHeapExtendSize(
...
//!           MOS_STATUS_SUCCESS if success, else fail reason
//!
MOS_STATUS CodechalCencDecodeNext::AssignSpaceInStateHeap(
    CODECHAL_CENC_HEAP_INFO *heapInfo,
    HeapManager             *heapManager,
    MemoryBlock             *block,
    uint32_t                size)
...
MOS_STATUS CodechalCencDecodeNext::AddHucDummyStreamOut(
    PMOS_COMMAND_BUFFER cmdBuffer)
{
    MOS_STATUS eStatus = MOS_STATUS_SUCCESS;
    ...
    flushDwParams       = {};
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MI_FLUSH_DW)(cmdBuffer));
    ...
    imemParams                  = {};
    imemParams.kernelDescriptor = VDBOX_HUC_DECODE_DECRYPT_KERNEL_DESCRIPTOR;

    // set HuC DMEM param
    auto &dmemParams             = m_hucItf->MHW_GETPAR_F(HUC_DMEM_STATE)();
    dmemParams                   = {};
    dmemParams.hucDataSource     = &m_hucDmemDummy;
    dmemParams.dataLength        = m_dummyDmemBufSize;
    dmemParams.dmemOffset        = HUC_DMEM_OFFSET_RTOS_GEMS;

    streamObjParams.HucProcessing   = true;
    streamObjParams.HucBitstreamEnable = 1;

    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_IMEM_STATE)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MFX_WAIT)(cmdBuffer));
    ...
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_DMEM_STATE)(cmdBuffer));
    ...
    auto &startCmdParams            = m_hucItf->MHW_GETPAR_F(HUC_START)();
    startCmdParams                  = {};
    startCmdParams.lastStreamObject = true;
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_VIRTUAL_ADDR_STATE)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_IND_OBJ_BASE_ADDR_STATE)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_STREAM_OBJECT)(cmdBuffer));
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_START)(cmdBuffer));
...
MOS_STATUS CodechalCencDecodeNext::PerformHucStreamOut(
...
//! \return   MOS_STATUS
//!           MOS_STATUS_SUCCESS if success, else fail reason
MOS_STATUS CodechalCencDecodeNext::AssignAndCopyToSliceLevelBb(
    uint8_t bufIdx)
{
...
MOS_STATUS CodechalCencDecodeNext::Execute(void *params)
{
    MOS_STATUS                   eStatus = MOS_STATUS_SUCCESS;
    ...
    mhw::vdbox::huc::HucMmioRegisters  *mmioRegisters;
    ...
    CENC_DECODE_CHK_COND((m_vdboxIndex > m_mfxItf->GetMaxVdboxIndex()), "ERROR - vdbox index exceed the maximum");
    mmioRegisters = m_hucItf->GetMmioRegisters(m_vdboxIndex);
    ...
    if (MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
    {
        // for M12 small memory bar config, remove m_resSlcLevelBatchBuf which is temp beffer between HuC and Decoder
    ...
    auto &dmemParams             = m_hucItf->MHW_GETPAR_F(HUC_DMEM_STATE)();
    dmemParams                   = {};
    dmemParams.hucDataSource     = &m_resHucDmem[m_urrDecryptBufIdx];
    dmemParams.dataLength        = MOS_ALIGN_CEIL(m_dmemBufSize, MHW_CACHELINE_SIZE);
    dmemParams.dmemOffset        = HUC_DMEM_OFFSET_RTOS_GEMS;
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_DMEM_STATE)(&cmdBuffer));
    ...
    if (MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
    {
        // for M12+, HuC directly write 2nd level batch buffer, need to assign heap resource and offset here.
    ...
    {
        // for not M12+, HuC write 2nd level batch buffer to m_resSlcLevelBatchBuf as temp
    ...
    auto &indObjParams          = m_hucItf->MHW_GETPAR_F(HUC_IND_OBJ_BASE_ADDR_STATE)();
    indObjParams                = {};
    indObjParams.DataBuffer     = decryptionParam->presDataBuffer;
    ...
    indObjParams.DataSize              = decryptionParam->dwDataBufferSize;
    ...
    indObjParams.StreamOutObjectBuffer   = resHeap;
    indObjParams.StreamOutObjectSize     = heapInfo->BitstreamBlock.GetSize();
    indObjParams.StreamOutObjectOffset   = heapInfo->BitstreamBlock.GetOffset();

    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_IND_OBJ_BASE_ADDR_STATE)(&cmdBuffer));
    CENC_DECODE_VERBOSEMESSAGE("Ind obj from MSDK, size = %d.", indObjParams.DataSize);
    ...
    auto &streamObjParams                         = m_hucItf->MHW_GETPAR_F(HUC_STREAM_OBJECT)();
    streamObjParams                               = {};
    streamObjParams.IndirectStreamInDataLength    = decryptionParam->dwDataBufferSize;
    streamObjParams.IndirectStreamInStartAddress  = 0;
    streamObjParams.HucProcessing                 = true;
    streamObjParams.IndirectStreamOutStartAddress = 0;
    streamObjParams.HucBitstreamEnable            = 1;
    ...
    if (m_noStreamOutOpt)
    {
        streamObjParams.StreamOut = 0;
    }
    else
    {
        streamObjParams.StreamOut = 1;
    }
    streamObjParams.EmulationPreventionByteRemoval = 1;
    if (m_lengthModeDisabledForDash)
    {
        streamObjParams.EmulationPreventionByteRemoval = 0;
        streamObjParams.Drmlengthmode                  = 0;
    }
    else if (m_dashMode)
    {
        streamObjParams.Drmlengthmode = 1;
    }
    else  // Widevine Classic
    {
        streamObjParams.StartCodeSearchEngine  = 1;
        streamObjParams.StartCodeByte0         = 0;
        streamObjParams.StartCodeByte1         = 0;
        streamObjParams.StartCodeByte2         = 1;
    }
    CENC_DECODE_CHK_STATUS(m_hucItf->MHW_ADDCMD_F(HUC_STREAM_OBJECT)(&cmdBuffer));
    ...
    CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MI_STORE_REGISTER_MEM)(&cmdBuffer));
    ...
    /* 
    *  m_miItf->MHW_ADDCMD_F(MI_BATCH_BUFFER_END) only add 'MI_BATCH_BUFFER_END' cmd, but the Epilog will miss,
    *  m_miItf provides func AddMiBatchBufferEnd which will call m_miItf->MHW_ADDCMD_F(MI_BATCH_BUFFER_END),
    *  also contains Epilog and other process logic for buffer end.
    */
    CENC_DECODE_CHK_STATUS(m_miItf->AddMiBatchBufferEnd(&cmdBuffer, nullptr));
...
MOS_STATUS CodechalCencDecodeNext::AllocateDecodeResource(
    uint8_t bufIdx)
{
    MOS_STATUS              eStatus = MOS_STATUS_SUCCESS;
...
MOS_STATUS CodechalCencDecodeNext::StartStatusReport(
    PMOS_COMMAND_BUFFER cmdBuffer)
{
    MOS_STATUS                   eStatus = MOS_STATUS_SUCCESS;
...
MOS_STATUS CodechalCencDecodeNext::EndStatusReport(
    PMOS_COMMAND_BUFFER cmdBuffer)
{
    MOS_STATUS                        eStatus = MOS_STATUS_SUCCESS;
    PCODECHAL_CENC_STATUS_BUFFER      decryptStatusBuf;
...
MOS_STATUS CodechalCencDecodeNext::ResetStatusReport(
    bool nullHwInUse)
{
    MOS_STATUS                   eStatus = MOS_STATUS_SUCCESS;
    PCODECHAL_CENC_STATUS_BUFFER decryptStatusBuf;
    ...
    if (!m_osInterface->bEnableKmdMediaFrameTracking)
    {
        ...
        auto &batchBufferEndParams = m_miItf->MHW_GETPAR_F(MI_BATCH_BUFFER_END)();
        batchBufferEndParams       = {};
        CENC_DECODE_CHK_STATUS(m_miItf->MHW_ADDCMD_F(MI_BATCH_BUFFER_END)(&cmdBuffer));
...
MOS_STATUS CodechalCencDecodeNext::GetStatusReport(
    void        *status,
    uint16_t    numStatus,
            ...
            if (localCount == 0 || localCount > globalCount)
            {
                ...
                if (decryptStatus->dwHWStoredData == CODECHAL_STATUS_QUERY_END_FLAG)
                {
                    ...
                    if (!(decryptStatus->dwErrorStatus2Reg & decryptStatus->dwErrorStatus2Mask) ||
                        (decryptStatus->dwErrorStatusReg & m_hucItf->GetHucErrorFlagsMask()))
                    {
                ...
                CODECHAL_DEBUG_TOOL(
                    ...
                    if (MEDIA_IS_SKU(m_skuTable, FtrCencDecodeRemove2ndLevelBatchBufferCopy))
                    {
                        // for M12+, need dump slice level batch buffer from heap
                    ...
                    {
                        // for not M12+, need dump 2nd level batch buffer from temp buffer
                        CENC_DECODE_CHK_STATUS(m_debugInterface->DumpHucRegion(
...
MOS_STATUS CodechalCencDecodeNext::Initialize(
    ...
    CodechalSetting  *settings)
{
    ...
    m_mhwInterfaces     = MhwInterfacesNext::CreateFactory(params, m_osInterface);
    CENC_DECODE_CHK_NULL(m_mhwInterfaces);
    CENC_DECODE_CHK_NULL(m_mhwInterfaces->m_cpInterface);
    m_cpInterface = dynamic_cast<MhwCpBase *>(m_mhwInterfaces->m_cpInterface);
    CENC_DECODE_CHK_NULL(m_cpInterface);
    m_hwInterface = CodechalHwInterfaceNext::Create(m_osInterface, CODECHAL_FUNCTION_CENC_DECODE, m_mhwInterfaces, false);
    CENC_DECODE_CHK_NULL(m_hwInterface);

    m_miItf = m_hwInterface->GetMiInterfaceNext();
    CENC_DECODE_CHK_NULL(m_miItf);
    m_hucItf = m_hwInterface->GetHucInterfaceNext();
    CENC_DECODE_CHK_NULL(m_hucItf);
    m_mfxItf = m_hwInterface->GetMfxInterfaceNext();
    CENC_DECODE_CHK_NULL(m_mfxItf);
    m_hcpItf = m_hwInterface->GetHcpInterfaceNext();
    CENC_DECODE_CHK_NULL(m_hcpItf);
    ...
    if (MEDIA_IS_SKU(m_hwInterface->GetSkuTable(), FtrMemoryCompression))
    {
        ...
        ReadUserSetting(m_userSettingPtr,
            ...
            true,
            true);
        m_mmcEnabled = outValue.Get<int32_t>();
    }
...
#if (_DEBUG || _RELEASE_INTERNAL)
    {
        ...
        ReadUserSettingForDebug(m_userSettingPtr,
            ...
            "Decrypt Delay Start",
            MediaUserSetting::Group::Frame);
        m_decryptDelayStart = outValue.Get<uint32_t>();
    }

    {
        ...
        ReadUserSettingForDebug(m_userSettingPtr,
            ...
            "Decrypt Delay Step",
            MediaUserSetting::Group::Frame);
        m_decryptDelayStep = outValue.Get<uint32_t>();
    }
    ...
    m_HuCFamily        = m_hucItf->GetHucProductFamily();
...
#if (_DEBUG || _RELEASE_INTERNAL) && defined(CODECHAL_HUC_KERNEL_DEBUG)
    {
        ...
        ReadUserSettingForDebug(m_userSettingPtr,
            ...
            "HuC Kernel Debug Level",
            MediaUserSetting::Group::Frame);
        ucHucKernelDebugLevel = (uint8_t)outValue.Get<int32_t>();
    }
#endif  // (_DEBUG || _RELEASE_INTERNAL)
    ...
    CENC_DECODE_CHK_STATUS(m_hwInterface->GetHucStateCommandSize(
...
#if (_DEBUG || _RELEASE_INTERNAL)
    {
        ...
        ReadUserSettingForDebug(m_userSettingPtr,
            ...
            "Enable Aes Native",
            MediaUserSetting::Group::Frame);
        m_enableAesNativeSupport = outValue.Get<int32_t>();
    }
#endif  // (_DEBUG || _RELEASE_INTERNAL)
...
MOS_STATUS CodechalCencDecodeNext::UpdateDataBuffer(void *buffer, uint32_t size)
{
    MOS_STATUS  eStatus = MOS_STATUS_SUCCESS;
...
void CodechalCencDecodeNext::SetReportSizeApp(uint32_t size)
{
    m_standardReportSizeApp = size;
}

void CodechalCencDecodeNext::FixEmulationPrevention(uint8_t* &outBitStreamBuf, uint32_t* byteWritten)
{
...
void CodechalCencDecodeNext::WriteBits(
...
MOS_STATUS CodechalCencDecodeNext::EncodeExpGolomb(
    uint32_t uValue, uint8_t valueValidBits,
